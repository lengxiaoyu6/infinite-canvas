...
async function withLoadedImage<T>(dataUrl: string, render: (image: HTMLImageElement) => T) {
    let lastError: unknown = null;
    for (const src of getImageRequestUrls(dataUrl)) {
        try {
            return render(await loadImageFromSrc(src));
        } catch (error) {
            lastError = error;
        }
    }
